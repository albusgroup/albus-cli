...
Pass `--idempotency-key` to make a run retry-safe. The JSON output carries the
effective `idempotency_key`, whether supplied by the caller or generated by
the server — absent only if something between the CLI and Albus strips the
response header on a run that supplied no key.
